...
public abstract class Traversal {
    ...
    protected Traversal(Graph G, Queue<Integer> fringe) {
        _G = G;
        _fringe = fringe;
        mark = new boolean[_G.maxVertex() + 1];
    }
    ...
    public void traverse(Collection<Integer> V0) {
        // FIXME
        // Add just first element of V0 to fringe, then add its children
        Object[] V;
        V = V0.toArray();
        for (int i = 0; i < V0.size(); i += 1) {
            _fringe.add((int) V[i]);
        }
        ...
        while (!_fringe.isEmpty()) {
            ...
            if (!marked(vertex)) {
                ...
                Iteration<Integer> iter = _G.successors(vertex);
                while (iter.hasNext()) {
                    int next = iter.next();
                    if (!marked(next)) {
                        _fringe.add(next);
